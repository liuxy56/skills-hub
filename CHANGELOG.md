...
- **Consistent deletion confirmations**: Single and bulk Skill deletion now explain app recycle-bin retention. Shared confirmation dialogs fix permanent-delete spacing, distinguish recoverable and permanent actions, and provide consistent keyboard and busy states.
- **One recycle bin for Skill deletion**: Save the app recovery snapshot before removing managed files and tool targets directly. Whole-Skill deletion no longer duplicates content in the system trash; other removal operations retain their existing protections.
- **Consistent recycle bin tool counts**: Reuse Skill card filtering and tool labels, count distinct synced tools, and restore only displayed relationships while keeping historical relationships disabled.
- **Consistent recycle bin spacing**: Removed duplicate page padding so content aligns with other pages at every sidebar and window size.
- **Compact recycle bin layout**: Combined title, count, help, and search in one header; made list columns adapt to available space and simplified restoration details.
- **Recycle bin help and detail dismissal**: Added an on-demand explanation of retention, startup and daily cleanup, restoration, and local storage. Closing details expands the list and keeps details closed during searches.
- **Bounded device sync history**: Device sync retains the latest 100 meaningful runs, including changes, conflicts, failures, and resolved-conflict activity. Successful no-change runs remain summarized as the latest check without consuming history; interrupted `running` rows stay out of history and are cleaned up on retry. Pruning also removes per-run details and applies to history left by earlier versions. The history view loads the retained records in two batches and stops at the retention boundary.
- **Safe per-device sources and same-name downloads**: Device sync preserves each computer's existing local or Git source binding without republishing it as shared metadata. Scheduled source updates use the last imported source snapshot, so an unchanged source cannot overwrite newer device-sync content while real source changes and explicit updates still apply. Multiple incoming Skills with the same name receive distinct managed directories without touching pre-existing folders.
- **Device-local source ownership**: Newly synced local Skills have no local source binding instead of pointing to their managed copy. Existing installations retain their own source. Unbound Skills stay usable and receive device sync updates, while source updates are disabled and excluded from automatic updates. Legacy imported bindings are cleared on the next sync.
- **Complete recycle-bin restoration**: Deleted Skills retain their saved descriptions and tag names. Restoring recreates tag associations atomically, refreshes tag counts, and keeps recoverable content on failure. Legacy entries recover descriptions from SKILL.md when available; tags missing from old backups cannot be reconstructed.
