...
### 优化
- **回收站保存位置**：说明中展示当前设备的手动删除与设备同步删除的文件备份实际路径，支持选中复制查找；数据库由应用内部管理。
- **回收站错误提示国际化**：加载、恢复和永久删除失败提示支持中英韩三语，为恢复位置占用、备份丢失等情况提供操作指引。
- **明确删除按钮文案**：单个和批量 Skill 操作统一叫“删除”，弹窗上方说明删除后进入 Skills Hub 本机回收站、保留 30 天。
- **统一删除确认界面**：修正单个及批量 Skill 删除的旧系统回收站说明；统一确认弹窗和按钮层级，修复永久删除弹窗贴边，补齐键盘和加载状态保护。
- **统一 Skill 删除回收流程**：先保存本机回收站文件与配置快照，再直接清理托管文件和工具目标；删除整个 Skill 不再重复送入系统废纸篓，其他移除操作保留原有保护。
- **统一回收站工具统计**：复用 Skill 卡片的工具筛选和名称，按已同步工具去重计数；恢复只重建展示的关系，其他历史关系保留为停用状态。
- **统一回收站边距**：移除重复的页面内边距，普通窗口、窄窗口及侧栏收起时均与其他页面对齐。
- **紧凑回收站布局**：标题、数量、说明和搜索合并为紧凑页头；列表按可用宽度调整列，详情区精简边框并支持长内容换行。
- **设备同步历史保留上限**：只保留最近 100 条有效同步记录，包括内容变化、冲突、失败和冲突处理记录；成功但无变化的同步仍只更新最近检查状态，不占用历史条数。中断遗留的“运行中”记录不会进入历史，并在重试时清理。裁剪时同步删除每次运行的变化明细，并能清理旧版本遗留的超量记录；页面分两批加载并在 100 条边界停止。
- **设备来源与同名下载安全**：设备同步保留每台电脑已有的本地目录或 Git 来源绑定，且不会把设备私有绑定重新写入共享元数据。定时来源更新以最近一次已导入内容为基准，来源未变化时不会覆盖较新的设备同步内容，来源确有变化或用户主动更新时仍可正常应用；多个同名 Skill 同批下载时使用不同托管目录，不覆盖已有目录。
- **本机来源归属**：首次同步的本地 Skill 不再把托管目录当成来源；已有安装保留本机来源。未绑定来源的 Skill 可正常使用及接收设备同步，界面明确提示并停用来源更新，自动更新也会跳过。旧版错误导入绑定在下次同步时清理。
- **完整恢复回收站信息**：删除时保存描述和标签，恢复时一并还原并刷新标签计数；失败时保留回收站内容。旧记录可从 SKILL.md 补回描述，未保存的旧标签无法自动重建。
- **同步可靠性修复**：冲突处理采用对应的仓库快照并保留脚本执行权限；实际处理的变化单独记录明细，不误报整个仓库同步完成。支持加载超过 50 条的历史。修复同步下载的本地 Skill 重复分发、删除和迁移目录问题，避免旧的中断同步记录误改后来安装的本地来源。
- **同步记录计数与明细**：已有 Skill 的单侧变化正确计为更新，排除内部拉取缓存并兼容旧快照。新记录可展开查看具体 Skill、变化类型和方向；旧版统计明确标为未校正，存在冲突的计划说明尚未执行。
- **本地来源路径按设备隔离**：本地 Skill 的来源路径不再进入同步清单，接收设备使用自己的托管目录，本机安装的来源关联保留。修复旧版导入的外来路径及对应的来源缺失异常，不清除其他异常；来源标记随删除、重装和身份合并正确维护。
- **修正新设备冲突处理误删**：解决单个 Skill 冲突不再推进整个库的同步基线；尚未同步到本机的项目会正常下载，不再被视为本机删除。逐项选择可跨重启保留，并在成功同步后失效。
- 修正“如何接入另一台设备”帮助入口，展示独立接入步骤及当前仓库和分支。
- **精简 Skill 异常提示**：异常图标移至名称后的复制按钮旁，点击浮层查看原因与处理建议，不再撑高卡片。
- **紧凑的设备同步与共享设备记录**：压缩同步状态、仓库和定时设置区域，设备列表展示同步记录，不再推断在线或版本状态。仓库通过单个 `devices.json` 记录设备，检查变化后立即更新本地缓存，兼容旧版设备并在过期推送后安全重试。
- **精简我的 Skills 顶部**：使用带数量的范围筛选替代统计卡片，移除重复的范围下拉框，仅在存在同步异常时显示紧凑的处理入口。

### 新增
- **独立回收站**：删除的 Skill 会在本机独立回收站保留 30 天。恢复时一并还原文件、描述、标签、启用状态、来源信息和工具同步关系；应用启动时及运行期间每 24 小时自动清理过期项目。
- **韩文界面语言**：新增覆盖应用界面的韩文翻译、三语言选择器及韩文偏好持久化；韩文用户查看应用更新时使用英文更新日志。
- **多设备 Skill 库同步**：新增基于 Git 的设备同步，支持 GitHub、GitLab 和 Gitee。默认通过浏览器完成账号授权，同时在高级设置中保留 Token/SSH；用户可选择已有私有仓库或在应用内创建仓库，并支持只读检查、手动立即同步及可选的启动时自动同步。
- **安全的三方比较与合并**：按稳定身份、来源或完全相同内容识别 Skill。双方修改不同文件时自动合并；重叠修改、删除与修改并发时，只暂停对应 Skill，由用户选择保留本机、使用远端或两份都保留。
- **本机恢复与历史记录**：内容变化和设备登记更新会创建 Git 提交；远端删除会将本机 Skill 移入保留 30 天的回收站；应用会记录同步运行、冲突和恢复状态。

### 修复
- **源文件缺失时的 Skill 详情**：来源异常提示不再占满详情工作区；本地原始来源不存在时，仍可正常查看中央仓库中的托管副本。
- **统一 Skill 异常状态**：设备同步不再清除来源更新错误，各页面刷新当前异常，工具分发待处理不再误报为全部成功。
- **分离云端同步与工具分发**：工具副本异常不再导致中央库同步失败；列表同步和设备同步共用受保护的副本刷新逻辑。设备同步页单独列出待处理工具并提供处理入口，冲突选择保留本机/远程两项并取消推荐。
- **共享目录同步与设备码反馈**：同一 Skill 共用工具目录时只更新一次文件，并更新所有副本记录；保留跨 Skill 覆盖保护。复制设备码成功后显示反馈，失败时提示并允许重试。
- **安装包 GitHub 授权配置**：打包前检查应用公开 Client ID，缺失或格式不合法时直接停止，避免安装后授权按钮不可用；本地可显式指定配置文件，仅提取这一项，不导入其他变量。
- **设备同步文件保留与工具副本**：回写保留本机排除上传的文件和链接，正常同步 `dist`/`build` 中的正式文档并迁移旧忽略规则；安全刷新工具复制副本，支持接受远端删除冲突并保留本机回收记录。元数据与副本基线统一事务提交，Python 字节码缓存不再被复制冲突检查误判为用户编辑。
- **设备同步失败原因可见**：顶部展示最近失败原因，同步记录可展开查看原因与处理建议，不再显示全零变化摘要。手动失败后立即刷新；新记录只保存白名单诊断码，旧错误与同步接口响应经安全处理后展示。
- **文本自动合并**：设备同步通过 libgit2 合并同一 UTF-8 文件中不重叠的修改；无法合并、二进制、超限文件、删除及元数据冲突仍需明确处理。远程拒绝推送时会停止同步，不会将结果应用到本机。
- **范围筛选与批量操作一致性**：标签和未打标签数量随范围、搜索更新，不受已选标签影响；批量操作及确认数量排除隐藏选择。筛选无结果时提供清除入口，从标签管理查看 Skills 时重置旧范围和搜索条件。
- **筛选后 Skill 安装范围**：Git 和本地导入弹窗现在只安装当前搜索结果中已勾选的 Skill，与显示的选中数量一致。隐藏项的勾选状态在清空搜索后保留，但被筛除时不会参与安装；当前没有选中有效 Skill 时禁用安装按钮。
- **嵌套 Git Skill 发现**：从仓库根地址或标准 `skills/` 目录链接添加时，现在可以识别 `mattpocock/skills` 等仓库的 `skills/<分类>/<技能>/SKILL.md` 结构。扫描限定在 `skills/` 内最多四层目录，遇到 Skill 即停止深入，并跳过目录符号链接及常见依赖、构建目录（[#129](https://github.com/qufei1993/skills-hub/issues/129)、[PR #131](https://github.com/qufei1993/skills-hub/pull/131)）。
- **删除操作二次确认**：移除自定义工具和手动清理 Git 缓存前，新增应用内二次确认，并明确说明会删除什么、会保留什么。移除自定义工具不会删除对应工具目录中的 Skill 文件；清理 Git 缓存不会影响已安装的 Skills 或本地原始目录。
- **安全迁移、移除与自动替换 Skills**：修改 Skills 中心存储位置时，会拒绝与工具同步目录、本地原始源目录或当前存储目录树重叠的路径；已有 Skills 需要明确确认后才会迁移，底层同步也会独立拦截源目录与目标目录重叠。修改自定义工具目录时，也会阻止同步目标与本地原始 Skill 重叠，保持原配置和文件不变，并显示可操作的本地化提示。删除整个 Skill 使用本机回收站；取消同步、停用和迁移清理保留系统废纸篓保护，自动更新不使用系统废纸篓。自动更新会跳过内容未变化的 Skill，并分别报告“已检查、无需更新、已更新、更新失败”；同时使用包含文件权限的无歧义内容指纹，以跨进程锁串行化整轮更新，并对共享物理目标去重；在进程正常运行期间，任一校验或数据库写入失败都会整体回滚，非预期目标内容及回滚期间的并发写入会原样保留并等待人工处理。本地原始源目录始终保留（[#123](https://github.com/qufei1993/skills-hub/issues/123)、[PR #125](https://github.com/qufei1993/skills-hub/pull/125)）。
...
### 安全
- **私钥内容检查**：设备同步对完整文件（包括自动合并结果）检查常见私钥标记，不再只扫描前 4 KB；错误信息不包含检测到的私钥内容。
- **按仓库可见性读取凭据**：公开 HTTPS 仓库检查不读取系统凭据；私有/内部仓库读取和上传分别鉴权。可见性未知时暂停后台检查，公开仓库上传需确认。
- **系统凭据存储**：HTTPS 访问令牌保存在 macOS 钥匙串、Windows 凭据管理器或 Linux Secret Service 中，不写入同步仓库或 SQLite；SSH 仓库使用系统 SSH Agent。
- **明确的同步边界**：只同步 Skill 内容和可移植元数据。工具目标、项目路径、应用设置、缓存与凭据始终留在本机；私钥文件会被拒绝，常见密钥与构建文件会被排除。

## [0.9.1] - 2026-08-29

### 变更
- **明确授权支持状态**：浏览器授权不可用时明确提示当前版本暂不支持，隐藏不可用的授权按钮，并展开 Token/SSH 连接方式。
- **统一设备名称**：本机改名在下次同步时写入共享设备记录，其他设备拉取后显示同一名称；兼容已有本机备注，忽略对其他设备设置的旧备注，仅允许修改本机名称。
- **同步范围说明**：明确文件、描述、标签、Git 历史与设备记录的同步范围，说明仅保存在本机的数据及其他设备获取变化的时机。
- **侧边栏分组**：设备同步移至管理中心首位，回收站移至末尾；工作区保留我的 Skills 和添加 Skills。
- **回收站说明与详情收起**：点击说明可查看保留期限、启动及每日清理、恢复内容和本机存储规则；关闭详情后列表铺满宽度，搜索时保持收起。
