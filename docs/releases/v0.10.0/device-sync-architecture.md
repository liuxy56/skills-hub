...
- 用户明确开启自动检查后，应用启动时执行只读检查；也可以在同步页手动检查。
- 本机文件变化只更新“待同步”状态，不立即写入远端。
- 点击“立即同步”后执行拉取、比较、合并、提交和推送。
- 自动同步支持二选一：按间隔（默认 15 分钟，整数范围 5～43200 分钟），或每天本机时间 HH:MM（默认 09:00）。保存后生效，只在应用进程运行期间执行，不安装系统计划任务。
- 按间隔从启动/变更计划或上次同步结束后计时；每天定时从下一时间点执行。进程存活期间休眠错过任务，恢复后最多补一次，不补跑队列。调度每 15 秒检查本地元数据，到期才访问凭据和远端。
- 手动与自动同步共用互斥锁；执行前再次核对开关和计划。忙碌或有未解决冲突时跳过且不读取凭据，解除后恢复。失败指数退避：间隔模式从所选间隔的两倍开始，每日模式从 10 分钟开始，最多 1 小时（用户间隔大于 1 小时则不缩短）。成功后恢复正常计划。
- 无内容变化不增加同步历史、不弹通知；有变化或失败保留结果，完成事件刷新界面。凭据只保存在系统安全存储。
- 配置 JSON 新增可选 auto_sync_schedule（mode=interval + minutes，或 mode=daily + time）；旧配置缺少该字段时不启用周期调度，必须重新开启并保存，避免把旧版启动授权扩大为周期访问。无需数据库版本升级。
...
- 其他设备同步删除后，先保存本机回收站文件及配置快照，再删除托管记录并直接清理中央副本和工具目标，不同时写入系统废纸篓。详见[独立回收站设计](standalone-recycle-bin.md)。
...
连接设置页分别说明 Skill 文件、描述和标签，Git 内容变更历史，以及接入设备和最近同步时间。冲突处理不再作为同步内容列出。本地来源路径、工具同步关系、项目关联及回收站备份仅保存在当前设备。底部提示其他设备连接同一仓库并执行同步后才会获取变化，避免重复同步范围说明。以上文案提供中、英、韩三种语言。

### 设备名称共享

设备只能修改自己的名称。保存沿用本机 settings 中 `device_sync.device_alias.<设备ID>` 配置以兼容已有数据；该值现在作为设备的有效名称，而非仅供本机展示的备注。保存不访问凭据，也不立即启动同步。下一次同步将有效名称写入 `devices.json` 的 `name`，其他设备检查变化或同步后读取。设备 ID 不变。

已有的本机自身备注自动作为共享名称使用；旧版对其他设备的本地备注不再覆盖仓库名称。清空名称恢复自动获取的默认名称，并在下次同步发布。旧版客户端仍可读取相同的 JSON 格式；若设备回退旧版客户端，其后续同步可能重新发布默认名称。

### 浏览器授权不可用时的展示

根据后端返回的授权可用状态展示界面。当前版本未配置 Gitee 授权服务时，明确提示“当前版本暂不支持 Gitee 浏览器授权”，隐藏首次授权按钮，并展开 Token/SSH 连接方式。其他平台授权不可用时同样处理；配置可用时继续提供浏览器授权。该提示描述当前构建能力，不表示 Gitee 平台本身不支持 OAuth。
